...
| MOD-06 | Attendance, selfie, geotag, adjustments | `lib/attendance*.ts`, attendance pages/components/migrations | Critical | In progress — BUG-143 effective-date/minute-boundary correction is deployed/live-verified; BUG-128 is deployed; BUG-125 schema decision plus signed-in/multi-device physical QA remain pending |
...
| MOD-14 | Revenue/restaurant/manager dashboards | dashboard pages, competition/capacity libs | High | In progress — BUG-144 leader separation/daily-achievement count/average-sales infographic reproduced and active locally; BUG-126/127 deployed; signed-in live-data pending |
