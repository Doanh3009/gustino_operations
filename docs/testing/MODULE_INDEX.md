...
| MOD-06 | Attendance, selfie, geotag, adjustments | `lib/attendance*.ts`, attendance pages/components/migrations | Critical | In progress — BUG-052 policy/RPC and BUG-054 exact-day/employee correction list deployed; signed-in/iPhone/visual integration pending |
| MOD-07 | Payroll, KPI, commission | `lib/payroll.ts`, `lib/commission.ts`, admin/records pages, migrations | High | In progress — autosave, per-receipt aggregation and daily-detail source defects fixed; monthly/weekly semantics and finalization lock need business confirmation |
...
| MOD-10 | Inventory movements and stock reports | `InventoryPage.tsx`, focused inventory in `AdminPage.tsx`, `lib/store.ts`, inventory migrations | Critical | In progress — BUG-055 deployed; BUG-057 handover/POS reconciliation and six-sheet export pass local regression/build but are intentionally not deployed; live visual/workbook verification pending |
| MOD-11 | Kitchen production/batches | `KitchenPage.tsx`, store/schema/migrations | High | In progress — status-button mapping checked; cancellation reason needs business confirmation |
...
| MOD-13 | Daily shift report/finalization/archive | `ReportPage.tsx`, `ReportArchivePage.tsx`, `lib/reportSync.ts`, `lib/n8nReports.ts`, report APIs/RPCs | Critical | In progress — report fixes are deployed; BUG-057 read-only use of opening/closing balances passes locally without changing handover; first midnight cron/UI/n8n verification pending |
