...
- Current production deployment: `dpl_9zfvjMPrFbzzuFrzF5P3mnk4moDM`, aliased to `https://gustino-operations.vercel.app`, serves `index-C9vho61t.js`/`index-CCcaMT4s.css` and `AdminPage-DoM6UbAd.js`. Live verification confirms the intended Supabase bundle, warehouse sale-out table/export, competition classification, management realtime fallback and Admin attendance UI. Midnight auto-close remains protected/configured; its first real scheduled execution is pending.
