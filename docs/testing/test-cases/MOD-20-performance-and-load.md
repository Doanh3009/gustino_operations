...
| MOD20-TC-ORDERS-02 | View the six-column order report on a narrow phone | The on-screen table becomes horizontal label/value records; no name/date/requester is broken into one-character columns, while the 1080×1350 export remains a six-column table | Pre-fix test failed; focused/related regressions, TypeScript, isolated guarded build and live asset/API checks Passed; signed-in device visual pending |
