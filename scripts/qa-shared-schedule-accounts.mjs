...
try {
  ...
  await adminPage.goto(`${baseUrl}/#/admin/employees`, { waitUntil: 'networkidle' })
  await adminPage.getByRole('heading', { name: 'Nhân sự & Chi nhánh' }).waitFor()
  await adminPage.getByRole('button', { name: 'Tạo mới', exact: true }).click()
  await adminPage.getByText('Tạo hồ sơ nhân viên', { exact: true }).waitFor()
