...
select jsonb_pretty(jsonb_build_object(
  'managers', (select jsonb_agg(to_jsonb(managers) order by manager_name) from managers),
  'branch_data', (select jsonb_agg(to_jsonb(branch_data) order by id) from branch_data)
)) as audit;
