...
if (attendancePage.includes('Chọn filter ảnh chấm công') || attendancePage.includes('selectedPhotoFilterOption.previewFilter')) failures.push('Chức năng filter ảnh vẫn còn trên giao diện chấm công.')
...
if (attendanceLib.includes('ATTENDANCE_PHOTO_FILTER_OPTIONS') || attendanceLib.includes('smoothingCanvas') || attendanceLib.includes('context.filter')) failures.push('Xử lý filter ảnh vẫn còn trong luồng đóng dấu chấm công.')
