...
if (!attendancePage.includes('AttendanceSelfieCamera') || !attendancePage.includes("facingMode, setFacingMode] = useState<'user' | 'environment'>('user')")) failures.push('Trang chưa có camera selfie filter mặc định dùng camera trước.')
...
if (!attendancePage.includes('Chọn filter ảnh chấm công') || !attendancePage.includes('selectedPhotoFilterOption.previewFilter')) failures.push('Ảnh xem trước chưa cho chọn và hiển thị filter chấm công.')
...
if (!attendanceLib.includes("const ATTENDANCE_PHOTO_ENHANCEMENT_FILTER = 'brightness(1.07) contrast(1.03) saturate(1.08)'")) failures.push('Ảnh chấm công chưa có filter tăng sáng/màu nhẹ theo yêu cầu mới.')
if (!attendanceLib.includes('ATTENDANCE_PHOTO_SMOOTHING_SCALE = 0.2') || !attendanceLib.includes('ATTENDANCE_PHOTO_SMOOTHING_OPACITY = 0.34') || !attendanceLib.includes('context.drawImage(smoothingCanvas')) failures.push('Preset Mịn da chưa có lớp làm mịn da baby tương thích Safari/iPhone.')
if (!attendanceLib.includes('context.filter = photoFilter.canvasFilter') || !attendanceLib.includes("context.filter = 'none'")) failures.push('Filter ảnh đã chọn chưa được áp dụng/reset trước khi đóng dấu chữ/GPS.')
