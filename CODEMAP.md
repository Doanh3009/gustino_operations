...
- **Cột bảng `gt-list` phải có TRẦN px.** Để tất cả `fr` thì trên màn rộng các cột bị kéo giãn ra hai mép — đọc một dòng phải quét mắt cả nghìn pixel. Mẫu chuẩn: cột có nghĩa dùng `minmax(0, <px>)`, cột **cuối** nuốt phần thừa bằng `minmax(120px, 1fr)`. Bảng tồn kho (`inventoryStockCols`) đã đổi theo. Hàng cũng gọn lại: padding dọc `var(--gt-2)`, `min-height: 48px`.
...
- Test cập nhật theo hợp đồng mới: `test-competition-single-board`, `test-employee-sales-capacity`, `test-kpi-ranking-reward-clarity`, `test-competition-leader-kpi-capacity-infographic` (đều đảo sang `doesNotMatch` cho dải tổng). Bộ offline: **101/122 PASS**; 21 đỏ đều đỏ sẵn từ trước (thiếu `src/lib/payroll.ts`, `lazyRoute` import không phần mở rộng, permission matrix, ReportPage/handover…). `tsc -b` + `npm run build` xanh.
