...
- Test: mới `test-inventory-admin-split-cards` (khoá cặp card ngang nhau, cấm gộp, cấm quay lại trần px, cấm `gridTemplateColumns` inline, khoá pastel chỉ ở đầu card). Cập nhật theo hợp đồng mới: `test-competition-single-board`, `test-employee-sales-capacity`, `test-kpi-ranking-reward-clarity`, `test-competition-leader-kpi-capacity-infographic` (đều đảo sang `doesNotMatch` cho dải tổng). Bộ offline: **102/123 PASS**; 21 đỏ đều đỏ sẵn từ trước (thiếu `src/lib/payroll.ts`, `lazyRoute` import không phần mở rộng, permission matrix, ReportPage/handover…). `tsc -b` + `npm run build` xanh.
