...
`styles.css` khai `input, select, textarea { width: 100%; min-height: 50px; padding: 10px 13px; border: 1.5px solid; border-radius: 12px }` (dòng ~2566) và nằm **SAU** `@import './ui.css'`. Mọi thuộc tính bộ `gt-` không khai lại đều rơi xuống ô nhập ⇒ ô cao 50px nằm trong khung 36px, chữ lòi ra ngoài viền; ô chi nhánh/ngày trong thanh lọc phình vỡ hàng, nhãn `CHI NHÁNH` gãy hai dòng. Đã reset đúng MỘT chỗ trong `ui.css` (`.gt-field > input/select`, `.gt-search input`) kèm hoàn lại mũi tên `select` (vì `background: transparent` xoá luôn `background-image` dùng chung) và ép lại **16px trên ≤900px** để iOS không zoom khi focus.
- **Thanh lọc**: trong `.management-workspace`, `.page.admin-page` bị ép `padding: 0` nên thẻ bo góc dính sát mép và đè đáy header. Nay `.management-workspace .gt-filterbar` là **thanh full-bleed có `border-bottom`**, không bo góc. Chỉ scope cho `management-workspace` — workspace Quản lý vẫn giữ padding `42px/4vw` của `.page`, thanh full-bleed ở đó sẽ thành gạch chân lơ lửng. `.admin-section-content > .gt-page` được thêm `padding-top`.
- **Màu chủ đạo**: chip đang chọn (`.gt-chips button.is-active`) đổi từ viên đen trung tính sang **lime thương hiệu `#a8d12d`** + chữ `#23320a`, đúng cặp màu của `gt-btn--primary`. Vòng focus của field/search cũng đổi sang lime thay vì xanh dương `--gt-info`.
- **Cột bảng trải quá rộng: chữa ở BỀ NGANG CARD, đừng chữa ở lưới cột.** Đã đi sai hai lần trong cùng một ngày: (1) để mọi cột là `fr` trên card rộng cả trang ⇒ cột kéo giãn ra hai mép, đọc một dòng phải quét mắt cả 1.300px; (2) chặn trần px + cột cuối nuốt phần thừa ⇒ cột sát nhau nhưng **bỏ trống hẳn nửa phải card**, chủ hệ thống: *"dồn cột lại vậy thì xấu quá"*. Cách đúng: **`SplitPair` / `.gt-split-2`** — bảng **Tồn kho** và **Hao hụt** đứng CẠNH NHAU, mỗi bảng ~nửa trang, `fr` chia đều là vừa khít. ⚠️ **Hai `<Surface>` độc lập, KHÔNG gộp hai bảng làm một** (yêu cầu nói rõ) — mỗi bảng giữ nguyên bộ lọc/biểu đồ riêng. ≤1180px tự xếp dọc. Hàng cũng gọn lại: padding dọc `var(--gt-2)`, `min-height: 48px`.
- **⚠️ KHÔNG track `auto` trong `--gt-cols`.** `.gt-list__head` và mỗi `.gt-list__row` là những **grid RIÊNG BIỆT**, không phải các hàng của cùng một grid. Track `auto` vì thế co theo nội dung của CHÍNH DÒNG ĐÓ — dòng `1,435 kg` rộng hơn dòng `310 g` ⇒ cột cuối lệch ⇒ mọi cột phía trước xê dịch, đúng lỗi "chữ không thẳng hàng". Mọi track phải xác định: `<N>px` hoặc `minmax(0, <N>fr)`. Đã quét sạch 10 lưới trong `AdminPage` / `DashboardPage` / `PromotionsPanel`. **Đừng lách bằng `subgrid`** — WebView Zalo cũ không hỗ trợ thì cả bảng sập thành một cột. Test `test-gt-list-no-auto-column` quét toàn `src/` và có chốt chống xanh giả.
- **Cặp card ngang nhau nay có ở hai màn**: Kho (Tồn kho ↔ Hao hụt) và **Tổng quan (Cần xử lý ↔ Tình hình chi nhánh)**. Bảng Hao hụt đã **bỏ cột "Lượt"** theo yêu cầu — số lượt còn ở tooltip biểu đồ và danh sách chi tiết.
- **Pastel (`Surface tone=`)**: 4 sắc `mint | rose | sky | sand` (token `--gt-mint/-line`…). **Chỉ nhuộm ĐẦU card** (`gt-section-head` hoặc `summary` của khối gấp) + viền; vùng dữ liệu giữ nền trắng, tô cả card là số liệu mất tương phản. Kho: Tồn kho `mint` · Hao hụt `rose` · Đối soát ca `sky` · Sổ phát sinh `sand`. Cột biểu đồ hao hụt đổi từ `#c8322f` gắt sang đỏ pastel `#e8908d` — mức cảnh báo vẫn nằm ở chữ đỏ đậm.
- **⚠️ Lưới cột phải đặt qua custom property `--gt-cols`, KHÔNG `style={{ gridTemplateColumns }}`.** Style inline thắng cả media query, nên bản cũ vô hiệu hoá luôn bố cục dọc ≤900px của `.gt-list__row` — bảng kho trên điện thoại vẫn là lưới ngang. Đã sửa cả hai chỗ trong `AdminPage`.
- **Màn Thi đua: bỏ dải tổng đầu bảng + đoạn mô tả** (yêu cầu chủ hệ thống). Gỡ `competition-overview` (4 thẻ: doanh thu kỳ / có ngày đạt KPI / năng suất TB đội / thưởng KPI) và `competition-leader-separation-note`, kèm CSS. Số liệu không mất chỗ đọc: chip `x/y có ngày đạt KPI` vẫn ở tiêu đề section, năng suất là CỘT trong bảng xếp hạng, số ngày đạt của từng người nằm trong drill-down "KPI theo ngày". **Dòng quy tắc `competition-reward-rule` giữ nguyên** — nó chi phối cột tiền bên dưới (§57). Tổng "N lượt ngày đạt chỉ tiêu" của cả kỳ thì mất hẳn khỏi màn, còn trong Excel KPI.
- Test: mới `test-admin-split-cards` (khoá cả hai cặp card ngang nhau, cấm gộp, cấm quay lại trần px, cấm `gridTemplateColumns` inline, khoá pastel chỉ ở đầu card) và `test-gt-list-no-auto-column` (cấm track `auto`, quét toàn `src/`). Cập nhật theo hợp đồng mới: `test-competition-single-board`, `test-employee-sales-capacity`, `test-kpi-ranking-reward-clarity`, `test-competition-leader-kpi-capacity-infographic` (đều đảo sang `doesNotMatch` cho dải tổng). Bộ offline: **103/124 PASS**; 21 đỏ đều đỏ sẵn từ trước (thiếu `src/lib/payroll.ts`, `lazyRoute` import không phần mở rộng, permission matrix, ReportPage/handover…). `tsc -b` + `npm run build` xanh.

## 62. Tách vai Admin ↔ Quản lý · gỡ nghẽn gửi báo cáo (2026-08-13) — ĐÃ DEPLOY PRODUCTION

**⚠️ ĐẢO phân vai so với §2. Đọc trước khi sửa `access.ts` hay nav.**

- **Quản lý = người VẬN HÀNH.** Nhận toàn bộ việc hằng ngày Admin đang làm: Tổng quan, Doanh thu, Kho, Chấm công, Thi đua, Đơn hàng, Nhân sự. `ADMIN_CONSOLE_ROLES` nay là `['admin','manager','supmt']`; các mục trước đây khoá bằng `canUseAdmin` trong `AdminPage` đổi sang **`canOperateConsole` (admin | manager)**. Route `manager-*` và `admin-accounts` cũng theo `canOperateConsole`.
- **Admin = console CẤU HÌNH no-code.** `defaultPageForRole('admin')` nay là **`control`** (không còn `management`). ControlCenterPage có tab: Dữ liệu nền · **Khuyến mãi** · **Mức KPI** · Phân quyền · Nhật ký. `canConfigureSystem` = chỉ admin — Quản lý vận hành chứ không đổi giá/SKU/mức KPI. `BranchKpiSettings` chuyển từ mục Thi đua sang đây.
- **Admin KHÔNG bị tước quyền xem console vận hành** (§86): nav của admin vẫn có đủ mục vận hành ở nhóm dưới, để đối chiếu khi có sự cố. Chỉ đổi mặc định vào đâu và thứ tự ưu tiên.
- **Doanh thu giữ trang `dashboard` (`ManagerDashboardPage`)** cho cả hai vai — chủ hệ thống muốn giữ thiết kế bảng số của Quản lý. `canAccessPage('dashboard')` nay là `canOperateConsole`, không còn `role === 'manager'`.
- **Gỡ nghẽn gửi báo cáo tự động** (`api/n8n-report-image.ts`): bỏ điều kiện `leader_id = eq.operator.id` trong `verifyClosedShiftAndSnapshot`. Trước đây **chỉ đúng ca trưởng đã đóng ca đó** mới gửi được báo cáo của ca đó — ca trưởng Ca 1 tan ca 15:20 rồi tắt máy là báo cáo Ca 1 kẹt vĩnh viễn: ca trưởng Ca 2 mở app, hệ thống phát hiện thiếu và chuyển sang màn Báo cáo, nhưng API trả 409. Nay bất kỳ người vận hành **cùng chi nhánh** cũng gửi được (`OPERATOR_ROLES` = shift_leader | shift_deputy | manager | admin), vẫn giữ chốt: ca phải closed, phải có snapshot, Ca 2 phải hết ca đang mở; người gửi ghi vào `n8nDelivery.queuedBy`.
  - **Còn lại chưa vá:** ảnh vẫn dựng bằng `html2canvas` trong TRÌNH DUYỆT, nên vẫn cần ít nhất một người mở app. Đường server-side (`puppeteer-core` + `@sparticuz/chromium` đã có trong deps) chưa dựng. Cùng giới hạn với tự chốt ca ở §60.

## 61. Khuyến mãi / giảm giá sản phẩm (2026-08-13) — ĐÃ DEPLOY PRODUCTION

- **Vì sao có:** `products` chỉ có MỘT cột `price`. Hạ giá một món trong một giai đoạn thì hoặc sửa cột đó (mất giá gốc) hoặc sửa tay `sales_receipt_items` — đúng việc đã phải làm với bánh hạt dẻ 06–13/08. Bảng `product_promotions` là **lớp ghi đè có thời hạn** đặt trên giá gốc.
- **Bất biến sống còn: khuyến mãi chỉ quyết định giá TẠI THỜI ĐIỂM BÁN.** `promotionalPriceFor(productId, basePrice, { branchId, date })` bắt buộc nhận ngày; `productSaleValues(productId, qty, options?)` **chỉ áp khuyến mãi khi caller truyền `options.date`**. Không truyền ⇒ giá gốc, hành vi y như trước. Lý do: hàm này cũng dùng để tính lại doanh thu quá khứ (túi phát nhân viên, KPI, báo cáo) — nếu mặc định "hôm nay" thì một chương trình chạy hôm nay sẽ **viết lại doanh thu tháng trước**. Test `test-product-promotions.mjs` khoá đúng điểm này.
- Ưu tiên: chương trình của ĐÚNG chi nhánh thắng chương trình toàn chuỗi; cùng mức thì giá thấp hơn thắng. Khuyến mãi đắt hơn giá gốc bị bỏ qua (cấu hình sai không được làm tăng giá khách).
- Điểm nghẽn giá: `SalesPage → productSaleValues → configuredProductPrice → productById().price`. Gắn khuyến mãi ở `productSaleValues` là POS, menu và lúc ghi hóa đơn tự động đúng giá. POS hiện giá gốc **gạch ngang** khi đang KM (`values.discounted`).
- UI: `pages/admin/PromotionsPanel.tsx` trong Control Center → tab **Khuyến mãi** (cạnh Dữ liệu nền vì đây là cấu hình giá, không phải thao tác vận hành). Nạp + realtime ở `App.tsx` cạnh `loadBranchKpiOverrides`.
- RLS: **ai cũng ĐỌC** (POS nhân viên phải biết giá KM, nếu không nhân viên bán một giá còn hệ thống ghi một giá khác), **chỉ admin GHI**.
- Dữ liệu production: `products.cake-box.price` trả về **36.000** (giá niêm yết), khuyến mãi 29.900 từ 06/08 không hạn nằm trong `product_promotions`. Doanh thu 06–13/08 đã chỉnh còn 5.621.200đ; backup giá gốc ở `price_adjustment_backup_20260813`.

## 60. Ca phó ngang ca trưởng · tự chốt ca khi quên · vá lỗi chỉnh KPI (2026-08-13) — ĐÃ DEPLOY PRODUCTION

**⚠️ ĐẢO LUẬT so với §2 và §55–§58. Đọc kỹ trước khi "sửa lại" bất cứ thứ gì ở đây.**

- **CHỦ CA = NGƯỜI BẤM NHẬN CA.** Ca trưởng và ca phó **ngang quyền vận hành**; ai vào ca thì bấm nhận ca, phiên ca mang đúng tên người bấm và **không có cơ chế nào tự chuyển quyền chủ ca**. Đã gỡ: `blockedAsDeputy`, toàn bộ `reclaimShiftForPrimaryLeader`, nhánh chặn ca phó trong `openShiftAfterLeaderCheckIn`, nhãn `[CA PHÓ ĐỨNG THAY]`, nút "Mở ca thay ca trưởng" ở màn Bàn giao. `isLeaderRegistration` **không còn loại ca phó**.
  - `isDeputyShiftLeader` vẫn giữ, nhưng **chỉ để chấm KPI theo chức danh** (`positionKpiKey`), không còn quyết định ai đứng tên ca.
  - Bộ dò ca trong `App.tsx` và `reconcileOperationalShift` nay nhận cả `shift_deputy`.
- **Nguyên nhân thật của lỗi 12/08 (Nguyễn Thị Yến) nằm ở DỮ LIỆU, không chỉ ở code:** hồ sơ của cô ấy có `role = 'staff'` trong DB (chức danh `Ca phó (8h)`, `employment_type = leader`). Role `staff` bị **RLS chặn sạch** các bảng vận hành + `canUseOperations` chặn ở UI ⇒ có lịch 14:15–22:15 đã duyệt, đã chấm công mà vẫn không nhận nổi Ca 2. **3/5 ca phó bị y hệt**: Nguyễn Thị Yến, Đặng Thị Khánh Linh, Mã Thị Thanh Trúc — đã `update profiles set role='shift_leader'` trên production 13/08. Đây là di chứng của bug `storedRole` mô tả ở §58: bản vá code đã có nhưng **hồ sơ cũ chưa được migrate lại**. Lần sau đổi luật lưu role, nhớ backfill hồ sơ đang tồn tại.
- **Tự chốt ca khi QUÊN chốt** — `lib/shiftAutoClose.ts` mới. Ca 1 → **15:30**, Ca 2 → **22:30** (muộn hơn mốc gửi báo cáo 15 phút). **TUYỆT ĐỐI KHÔNG TỰ GHI TỒN** (ràng buộc cứng của chủ hệ thống): ghi `closingBalances = {}` + dấu `[TỰ ĐỘNG CHỐT] … TỒN CUỐI CA CHƯA KIỂM ĐẾM`. Tự điền tồn theo sổ sẽ xoá sạch chênh lệch thật và làm phiếu bàn giao mang con số không ai đếm. Màn đối soát ca vốn đã hiểu tồn rỗng là "Chưa bàn giao / Chưa chốt" nên ca đó nổi lên đúng nhóm cần rà. Tồn đầu ca sau không bị ảnh hưởng — `buildOpeningBalances` đọc lại từ sổ kho thật.
- **Mốc doanh thu 15:15 KHÔNG đổi** (`shiftReportScope.ts`): hóa đơn tới 15:15 thuộc Ca 1, sau 15:15 thuộc Ca 2, cắt thuần theo đồng hồ, độc lập giờ mở/đóng ca. Ba mốc đừng lẫn: **15:15 chia doanh thu · 15:15/22:15 gửi báo cáo · 15:30/22:30 tự chốt ca**.
- **Lỗi "chỉnh KPI xong nó giật rồi trở lại như cũ" — hai nguyên nhân chồng nhau:**
  1. Form **ép** chọn `Ngày áp dụng`; Admin chọn hôm nay, nhưng bảng xếp hạng đang xem cả tháng ⇒ `activeBranchKpiOverrideFor` bỏ qua override với mọi ngày `< effectiveFrom` ⇒ số mới loé lên rồi tụt về mức mặc định. Nay **để trống = áp dụng mọi kỳ**; điền ngày chỉ khi muốn giữ nguyên số liệu kỳ cũ, và ô ngày nói rõ hệ quả.
  2. Ô nhập dựng `value` bằng `toLocaleString('vi-VN')` ngay trong lúc gõ ⇒ con trỏ văng về cuối mỗi phím; fallback `'0'` khiến không xoá trắng ô được. Thay bằng component `AmountInput` giữ nguyên chuỗi đang gõ, format lại khi rời ô.
- **Nghẽn gửi báo cáo tự động — đã xác định nguyên nhân, CHƯA vá:** `reportSchedule.ts` chỉ **đặt cờ**; ảnh báo cáo phải dựng bằng `html2canvas` trong trình duyệt của ca trưởng. Không ai mở app đúng giờ = không gửi. Repo đã có sẵn đường server-side chưa dùng (`api/n8n-report-image.ts` + `puppeteer-core` + `@sparticuz/chromium`) — đó là hướng gỡ.
- **Test đã viết lại theo luật mới** (5 file): `test-shift-owner-must-be-leader` (tên file giữ nguyên nhưng luật đã đảo — đọc header), `test-separated-shift-roles-20260810`, `test-deputy-and-attendance-rules`, `test-auto-second-shift-start`, `test-allday-registration-not-steal-shift`. Bộ offline: **94/113 PASS**, 19 đỏ đều là đỏ sẵn từ trước đợt này.

## 59. Redesign Information Architecture 4 màn quản trị (2026-08-13) — CHƯA DEPLOY

**Phạm vi:** Tổng quan · Doanh thu · Kho (cả màn quản trị lẫn màn vận hành) · Chấm công. Sidebar, header, auth, phân quyền, routing, API và database **không đổi** — chỉ đổi content area và cách tổ chức thông tin.

- **Design system mới `gt-`** — `src/ui.css` (tokens spacing `4/8/12/16/20/24/32/40`, màu navy/lime/amber/đỏ, radius 10–14, shadow rất nhẹ) + `src/components/ui/index.tsx` (`PageHeader`, `FilterBar`, `BranchSelector`, `DateField`, `DateRangeField`, `QuickDatePresets`, `Metric`, `SummaryLine`, `Surface`, `SectionHeader`, `StatusBadge`, `SearchInput`, `FilterChips`, `ViewSwitch`, `OverflowMenu`, `Drawer`, `ActionSheet`, `DataList/DataHead/DataRow`, `RankBar`, `AttentionList`, `EmptyState`, `ErrorState`, `SkeletonRows`, `ReconRow`). `styles.css` `@import './ui.css'`. **Tiền tố `gt-` cố ý tách khỏi 18k dòng CSS cũ** — màn chưa redesign vẫn chạy nguyên.
  - `DataList` = lưới cột trên desktop, dòng dọc trên mobile. Ô con đánh dấu `data-gt-primary` / `data-gt-label` / `data-gt-trailing` / `data-gt-span`. **Mỗi dòng chỉ được có ĐÚNG MỘT `data-gt-trailing`** — hai ô sẽ chồng nhau ở (cột 2, hàng 1).
  - `Drawer` dùng `createPortal`; ≤900px tự thành bottom sheet.
- **MỘT thanh lọc cho cả trang Quản trị, đổi theo section** (`AdminPage` `<FilterBar>`): Tổng quan → chi nhánh + preset; Kho → chi nhánh + **một ngày** (`inventoryDate`); còn lại → chi nhánh + khoảng ngày + preset (+ nút bước tháng cho Chấm công). Không section nào được dựng bộ lọc chi nhánh/ngày thứ hai.
- **Tổng quan mặc định HÔM NAY** (`initialRange` khi `initialSection` là `overview`). `admin/DashboardPage.tsx` viết lại: 4 metric có ý nghĩa vận hành (doanh thu / đơn / tiến độ KPI / chi nhánh), khối **Cần xử lý** (âm kho, hết hàng, ca chưa bàn giao, chấm công tự đóng, hao hụt, đơn chờ) → mỗi dòng có nút dẫn sang đúng section, danh sách chi nhánh compact, top 5 thi đua, 5 giao dịch gần nhất. Online users thành chip ở header. **% so kỳ trước chỉ hiện khi dữ liệu kỳ trước THỰC SỰ đã nạp** (hóa đơn chỉ nạp từ đầu tháng của `from`) — thiếu thì không hiện, không bịa.
- **Kho theo NGÀY (§34).** `calculateStock(movements.filter(m => m.shiftDate <= D))` = tồn cuối ngày D; giữ nguyên luật mốc kiểm kê / `<=` / hao hụt chế biến nên không lệch với màn ca trưởng. Ngày quá khứ đổi tên cột thành *Tồn cuối ngày*. Bấm SKU mở drawer đối chiếu (tồn đầu + nhập + thành phẩm − chế biến − bán − hao ± điều chỉnh = tồn cuối) + timeline + **cảnh báo checkpoint kiểm kê (§50)** khi ngày đang xem nằm trước một phiếu `count`.
- **`•••` là nơi ở mới của chức năng phụ, KHÔNG phải nơi xoá.** Doanh thu: giao dịch / doanh thu theo ngày / tất cả món / hiệu suất NV / bản lưu báo cáo. Kho: sổ phát sinh (giữ nguyên ranh giới `canOpenAdminConsole`), đối soát ca, hao hụt chi tiết, 2 nút Excel. Chấm công: Excel + Chứng từ công.
- **Chấm công: 3 CÁCH XEM cùng một dữ liệu** (`attendanceView` = summary/shifts/issues), không còn cặp mode "Theo ngày / Theo nhân viên" chồng lên bộ lọc chung. Chỉnh công / Xóa ca chuyển vào drawer chi tiết ca (cùng RPC, cùng bắt buộc lý do, cùng audit). `AttendanceAdjustmentArchive` thêm prop `mode: 'both' | 'supplement' | 'archive'` + `onChanged` để tách "Bổ sung công" (nút chính) khỏi "Chứng từ công" (`•••`).
- **Kho vận hành (`InventoryPage`)**: hàng tab ngang → nút **`+ Phát sinh kho`** mở `ActionSheet` 5 loại phiếu (không mất phiếu nào), header có ô Ngày, dòng SKU bấm được → drawer đối chiếu, `•••` cho sao chép/xuất ảnh/lịch sử. **Phiếu mới vẫn luôn ghi `shiftDate: today`** — nên khi đang xem ngày quá khứ, nút Phát sinh kho bị khoá kèm lời giải thích. Backdated entry (§47/§48) **chưa làm** — xem phần Còn thiếu.
- **Điều hướng section giữ đúng URL**: `onOpenSection` gọi `navigateAdminHash(adminRouteForSection(section))`.
- **Còn thiếu / cố ý chưa làm:** (1) ghi phiếu lùi ngày cho admin + preview ảnh hưởng (§47, §48) — cần đổi đường ghi `shiftDate` và tính lại running balance qua checkpoint, rủi ro dữ liệu cao nên tách thành đợt riêng; (2) `effective_at`/`created_at` (§46) hiện được biểu diễn bằng cặp `shift_date`/`created_at` sẵn có, chưa thêm cột; (3) `transaction_group_id` cho mẻ chế biến (§43) hiện dùng `documentId`.
- **Test:** 14 test hợp đồng UI được viết lại để khoá cam kết nghiệp vụ ở **vị trí mới** (attendance ×6, kho ×3, doanh thu/tổng quan ×4, hiển thị số ×1). Bộ test chạy offline: **94/113 PASS**. 19 test còn đỏ là **đỏ sẵn từ trước** đợt này (AttendancePage, ReportPage, ShiftHandoverPage, `access.ts`, `App.tsx`, thiếu `src/lib/payroll.ts`, `kpiEvidenceWorkbook` import `./lazyRoute` không phần mở rộng, `INVENTORY_EXCEL_QUANTITY_FORMAT` đang là `'General'`).

## 58. Quyền Ca phó, hiệu năng tải trang, gộp Xem công, quy tắc vắng (2026-08-10) — ĐÃ DEPLOY PRODUCTION

- **Ca phó truy cập ngang Ca trưởng.** Lỗi "set quyền Ca phó rồi mà vẫn không vào được" có nguyên nhân ở HAI tầng: (1) `storedRole()` trong `attendance.ts` ghi Ca phó xuống DB thành **`staff`**, mà enum `app_role` không hề có `shift_deputy` và mọi policy RLS bảng vận hành chỉ cho `shift_leader` ⇒ bị chặn ngay ở database; (2) `OPERATION_ROLES` chỉ có `shift_leader` ⇒ giao diện tự khoá họ ra ngoài. Nay `storedRole` lưu `shift_deputy → shift_leader` và `OPERATION_ROLES = ['shift_leader', 'shift_deputy']`. **Mô hình chuẩn: Ca phó dùng chung role DB `shift_leader`, phân biệt bằng `position_title`; đọc lên thì `normalizeRole` suy ra `shift_deputy` để tính KPI riêng.** Edge function `manage-employee` chỉ nhận enum thật nên client BẮT BUỘC quy đổi trước khi gửi — đừng bỏ `storedRole`.
- **Bất biến không đổi:** Ca phó vẫn KHÔNG đứng tên phiên ca khi chi nhánh đã xếp ca trưởng (`blockedAsDeputy`, `isDeputyShiftLeader`, `reclaimShiftForPrimaryLeader`). Ranh giới hai vai trò nằm ở CHỦ CA, không ở quyền vào trang.
- **Hiệu năng — thủ phạm số 1 của "web load rất lâu".** `pg_stat_statements` prod: lệnh đọc sổ kho theo chi nhánh chạy **114.680 lượt, trung bình 685 ms, tổng ~21,8 giờ CPU**. Bảng chỉ có 8.128 dòng — không phải do dữ liệu lớn: chỉ mục duy nhất là `(branch_id, shift_date DESC)` trong khi câu lệnh `ORDER BY created_at DESC, id DESC`, nên mỗi lượt phải quét hết dòng của chi nhánh rồi SORT lại. Hai bản vá: (1) chỉ mục `stock_movements_branch_created_idx (branch_id, created_at desc, id desc)` — plan đo lại **4,26 ms → 0,67 ms**, hết bước sort; (2) `FULL_MOVEMENT_REFRESH_TICKS` trong `App.tsx` từ 10 nhịp (2,5 phút) lên **120 nhịp (30 phút)**. Hạ nhịp an toàn vì lượt tải đầy đủ chỉ là lưới an toàn: phiếu mới về qua nhịp gia số 15 giây, phiếu bị xoá do bộ đếm 3 ngày + event realtime DELETE bắt, đổi trang/đổi chi nhánh vẫn tải đầy đủ.
- **Không set được trạng thái Nghỉ việc.** RPC `admin_update_employee_crm` chặn cứng `Employment end date is required`, cộng thêm `required` ở ô nhập ⇒ Admin không chuyển nổi hồ sơ khi chưa nhớ ngày. Migration `20260810_employment_end_date_optional.sql` bỏ ràng buộc; để trống thì lấy hôm nay theo giờ Việt Nam làm mốc (cần một mốc để `wasEmployedDuring` biết ẩn từ kỳ nào).
- **Nhân sự đã nghỉ** (`lib/employmentStatus.ts`) — hai câu hỏi khác nhau, đừng dùng lẫn: `hasLeftCompany` (đã nghỉ chưa → dùng cho bảng thi đua, mọi thứ mang tính hiện tại) và `wasEmployedDuring(employee, from, to)` (trong KỲ ĐANG XEM còn làm không → dùng cho báo cáo theo kỳ). Nhờ vậy **tháng cũ giữ nguyên số liệu, chỉ các kỳ sau mới ẩn họ đi**.
- **Gộp "Xem công" vào trang Chấm công** — bỏ mục riêng ở sidebar, thành tab `timesheet` trong `AttendancePage`; `MyTimesheetPage` tách ra `MyTimesheetContent` để dùng chung. Link cũ `#my-timesheet` vẫn chạy, mở thẳng đúng tab.
- **Quy tắc công:** có đăng ký + hết giờ không chấm công = **VẮNG**; không đăng ký = **ô trống**, không tính vắng. Chỉ tính vắng SAU giờ tan ca (`now > scheduledEnd`), đăng ký đã bị từ chối không tính là có lịch.
- **Đơn nghỉ việc làm lại**: lý do chọn sẵn + checklist bàn giao + số ngày báo trước + dòng thời gian trạng thái + bộ lọc/thống kê, và **quyết định duyệt/từ chối dùng biểu mẫu trong trang** thay cho `window.prompt` — prompt/confirm trần bị WebView Zalo nuốt (BUG-137). Admin duyệt xong tick một ô là hồ sơ tự chuyển sang Nghỉ việc.
- Test mới: `test-deputy-and-attendance-rules`. Test cập nhật theo chính sách mới: `test-separated-shift-roles-20260810` (bỏ assert "Ca phó không kế thừa quyền", thay bằng assert ranh giới CHỦ CA), `test-stock-and-sync-performance` (chốt nhịp 120 + chỉ mục).
- Release: build `index-DPxoW9i_.js` trùng hash bundle live; audit sau deploy không bảng nào giảm.

## 57. Mức KPI chỉnh trong giao diện + bảng thi đua cho nhân viên + đơn nghỉ việc (2026-08-10) — ĐÃ DEPLOY PRODUCTION

- **Mức KPI theo chi nhánh giờ sửa được trong giao diện.** Bảng `branch_kpi_formulas` (khoá `branch_id, position`) là **lớp ghi đè** đặt trên `POSITION_KPI_FORMULAS`; chi nhánh/vị trí chưa có dòng thì vẫn chạy đúng mức cứng như cũ ⇒ bật tính năng không đổi con số nào đang có. Registry đặt trong `commission.ts` (`setBranchKpiOverrides` / `resolvedPositionFormula` / `branchStaffing`), nạp một lần ở `App.tsx` + `AdminPage`, nên **không phải đổi chữ ký hàng chục lời gọi** `employeePeriodRevenueTarget` rải khắp Admin/Report/Dashboard. UI: `pages/admin/BranchKpiSettings.tsx` trong Quản trị → Thi đua nhân viên. RLS: ai cũng ĐỌC (nhân viên cần biết chỉ tiêu của mình), chỉ `admin` GHI.
- **Kỳ Vũng Tàu 01–15/07/2026 bị đóng băng khỏi override** (`isFrozenVungTauWindow`): số đối soát tháng 7 đã ký với chủ hệ thống, override không được sửa lại. Ngoài cửa sổ đó override thắng.
- Khối ghi chú "KPI Vũng Tàu theo giai đoạn" trong AdminPage **đã bỏ** theo yêu cầu chủ quán — số cứng trong đó nay nằm ngay trong bảng chỉnh. Chỉ giữ lại **một dòng** quy tắc `Chỉ tính thưởng KPI theo từng ngày` vì nó chi phối cột tiền bên dưới.
- **Bảng thi đua cho mọi nhân viên** — trang mới `#competition` (`CompetitionBoardPage.tsx` + `lib/employeeCompetitionBoard.ts`), mở cho mọi vai trò bán hàng. Cố ý rút gọn: **không có nút xuất Excel/ảnh, không đọc `attendance_records`/`shift_registrations`, không hiện giờ công – số ca – tiền thưởng của người khác.** Chỉ có tên, vị trí, doanh thu, %KPI, hạng, trong đúng chi nhánh của mình. Dùng CHUNG `employeeCompetitionPeriodRevenueTarget` với trang Quản trị nên số không lệch hai nguồn.
- **KPI cá nhân trong màn Xem công** (`#my-timesheet`): khối KPI tháng (doanh thu/chỉ tiêu/%/xếp loại) + KPI của đúng ngày đang chọn kèm thưởng ngày. Tải riêng khỏi bảng công — hỏng lượt đọc doanh thu thì vẫn còn đủ giờ công.
- **Đơn xin nghỉ việc** — bảng `resignation_requests`, lib `resignationRequests.ts`, trang `#resignation` dùng chung cho 3 góc nhìn. Nộp: `staff | shift_leader | shift_deputy`. Đọc: người nộp + admin/manager + supmt + **ca trưởng/ca phó CÙNG chi nhánh** (`is_branch_shift_leader` ràng `actor.branch_id = p_branch_id`, không đọc chéo chi nhánh). Duyệt: chỉ admin/manager; ca trưởng chỉ bấm "Đã nắm thông tin". Unique index `where status in ('pending','acknowledged')` ⇒ mỗi người tối đa một đơn đang mở.
- Gỡ `weekStart()` chết trong AdminPage: nó dựng ngày ở giờ máy rồi `toISOString()` — đúng mẫu BUG-138, để lại là bẫy cho lần sau.
- Test mới: `test-business-date-alignment` (đỏ-trước với code tiền-BUG-138), `test-branch-kpi-settings`, `test-employee-competition-board`, `test-resignation-requests`.
- Migration đã apply production: `20260810_branch_kpi_formulas.sql`, `20260810_resignation_requests.sql`. Frontend vẫn chịu được việc thiếu bảng (`isMissingTable` → chạy mức mặc định / danh sách rỗng) để môi trường khác không vỡ.

## 56. KPI daily-only + doanh thu lịch sử trước web (2026-08-10) — ĐÃ DEPLOY PRODUCTION

- Quyết định mới supersede phần thưởng ở mục 55: tiền KPI chỉ bằng tổng thưởng ngày; không cộng thưởng tuần/tháng/special. Tổng hợp tháng vẫn giữ doanh thu, target, tỷ lệ và ngày đạt để đối soát.
- `employee_kpi_revenue_adjustments` lưu nguồn doanh thu nhân viên trước khi chi nhánh dùng web, tách khỏi POS/kho. `kpiRevenueAdjustments.ts` đọc theo phạm vi và tạo adapter chỉ-trong-bộ-tính; `AdminPage` gộp adapter với POS cho KPI ngày, KPI tổng, xếp hạng, drill-down và Excel nhưng giữ `salesReceipts` gốc cho hóa đơn/kho.
- Dữ liệu owner 03–10/07: 41 dòng = 30.679.000đ. Production đã có 11.235.000đ; migration idempotent chỉ ghi 27 khoản thiếu = 19.444.000đ (Minh Lý 09/07 chỉ delta 33.000đ). Audit sau migration giữ nguyên 187 POS receipts trong cửa sổ.
- Test đỏ-trước `test-kpi-revenue-adjustments-20260810`, daily-only/reverse/ranking/Vũng Tàu/timezone/Excel/drill-down/competition regressions, TypeScript và build 728 modules Passed. Release `dpl_Ey2sk56RevSWUiHTbSHjJTYhUcg2`; live `index-D0_hpX2p.js` + `AdminPage-BDnJeKJU.js` có đủ marker adjustment/daily-only.

## 55. KPI Vũng Tàu mới + thưởng tháng (2026-08-10) — ĐÃ DEPLOY PRODUCTION

- `commission.ts` tách `shift_deputy` (Ca phó) khỏi `shift_leader` (Ca trưởng). Vũng Tàu: PT 550k ngày thường/650k cuối tuần/14,9m tháng; FT 1,05m/1,3m/28,8m; Ca phó 500k cả hai loại ngày/13m. Ca trưởng target cá nhân 0.
- `branchTeamPeriodRevenueTarget`: cơ cấu chuẩn Vũng Tàu 4 FT + 1 Ca phó + 1 Ca trưởng ⇒ team 4,7m ngày thường, 5,7m cuối tuần, 128,2m/tháng. `AdminPage` gom cùng nguồn doanh thu nhân viên (allocation + POS direct, chống trùng) thành team; Ca trưởng nhận 30k × số ca thực tế khi team đạt ngày. Bảng ca vận hành chia target ngày đều cho số phiên ca để tổng target phiên bằng target team ngày.
- Thưởng tháng được mở trong kỳ trọn tháng: Full-time/Ca phó 0,5/1/1,5/2/2,5m; Ca trưởng 1/2/3/4/5m. Most Improved và Perfect Month cộng dồn tự động theo ảnh. PG of the Month chỉ hiện `pendingBonus` vì không có bảng vi phạm kỷ luật; Admin phải xác nhận ngoài hệ thống trước khi cộng.
- Mọi tháng cũ/hiện tại được tính động theo khung mới khi mở lại, không migration/backfill. Override `employee_kpi_targets` cũ bị bỏ qua riêng cho Vũng Tàu; audit production hiện có 0 override. Excel KPI có cột thưởng tháng, đặc biệt, chờ xác nhận và chi tiết.
- Tests: `test-vung-tau-kpi-20260810`, `test-kpi-date-timezone`, reverse audit và regression thi đua/Excel. Audit SELECT-only production 01/07–10/08 xác nhận header=item, không doanh thu vô chủ/override.
- Release `dpl_2hkXxEmNwya1LyLBbr8ZU4J86kdy` READY/aliased tại `https://gustino-operations.vercel.app`. Live bundles `index-DokHgxpS.js`, `AdminPage-CoZJoLK4.js`, `commission-UPVEiE_r.js` chứa đúng toàn bộ hằng số trên và phép ngày UTC. Hai hồ sơ được owner xác nhận là Ca phó (`Đặng Thị Khánh Linh`, `Mã Thị Thanh Trúc`) đã đổi đúng `position_title = Ca phó (8h)` qua Edge function quản trị, đọc lại xác nhận thành công.
- Audit sau release: tháng 7 Vũng Tàu 64.389.000đ/1.084 hóa đơn; tháng 8 đến 10/08 là 28.970.000đ/536 hóa đơn; header=item, mismatch/unattributed/override đều 0. Tháng 7 tính lại cho hai Ca phó: 104,54% → 1,5 triệu và 113,8% → 2 triệu thưởng tháng; tháng 8 hiện chưa ai đạt bậc tháng.

## 54. KPI ngày bị lùi một ngày ở UTC+7 (BUG-138, 2026-08-10) — ĐÃ DEPLOY

- `commission.ts` trước đây tạo `YYYY-MM-DD` ở local midnight rồi gọi `toISOString()`: tại Việt Nam ngày bị lùi một ngày. Hậu quả: thứ Bảy dùng mức ngày thường, thứ Hai dùng mức cuối tuần; `isFullCalendarMonth` không nhận kỳ trọn tháng nên cộng mọi ngày thay vì lấy mốc KPI tháng cố định.
- Lỗi có từ commit `86d6442c` ngày 16/07/2026. Với kỳ 01–31/07, code cũ tạo 30/06–30/07; PG part-time Gold Coast dùng 16,7 triệu thay vì 13,9 triệu. Có thể ảnh hưởng % KPI/xếp hạng, thưởng ngày và thưởng tuần; thưởng tháng vẫn 0.
- Fix BUG-138 ở lớp ngày thuần: `Date.UTC`, `setUTCDate/getUTCDate`, `getUTCDay`. Sau đó owner phê duyệt business change Vũng Tàu riêng tại mục 55.
- Test đỏ-trước/xanh-sau: `scripts/test-kpi-date-timezone.mjs`. Audit SELECT-only tháng 7–8 đã hoàn tất; không sửa dữ liệu doanh thu/chấm công lịch sử. Bản kết hợp đã deploy trong release mục 55; live bundle xác nhận có `Date.UTC`, `setUTCDate`, `getUTCDay`.
...
## 50. Ca chia theo ĐỒNG HỒ + báo cáo tự gửi đúng giờ (2026-08-11) — CHƯA DEPLOY
> Chủ quán chốt: "đúng 15h15 dữ liệu bán hàng thuộc Ca 1, sau giờ đó thuộc Ca 2; không cần bấm gửi báo cáo, cứ đúng giờ sẽ gửi; việc chốt ca của ca trưởng hay bàn giao không liên quan nữa."

**Luồng nhận ca / mở ca / bàn giao GIỮ NGUYÊN** (`shiftAutoOpen.ts`, `operationalShiftAssignment.ts`, nút nhận ca ở §2 không đụng tới). Chỉ đổi hai thứ: ranh giới doanh thu và thời điểm gửi báo cáo.

### 50.1. Ranh giới doanh thu = 15:15, không còn bám phiên ca (`src/lib/shiftReportScope.ts`)
- `SHIFT_CUTOVER_TIME = '15:15'` (neo `+07:00` theo `businessDate`, KHÔNG theo giờ máy người xem). Ca 1 = từ đầu ngày tới **hết** 15:15; Ca 2 = **sau** 15:15 tới cuối ngày. Hóa đơn đúng 15:15:00 thuộc Ca 1.
- `sessionScopeWindow(target, sessions)` giữ nguyên chữ ký (mọi màn dùng chung một cửa: `ReportPage`, `shiftCompetition.ts`, `ShiftHandoverPage.receiptsInSession`), nhưng tham số `sessions` **không còn ảnh hưởng gì** — ranh giới thuần đồng hồ. Thêm `shiftClockWindow(businessDate, sequence)` và `shiftCutoverTimestamp(businessDate)`.
- **`timestampInScopeWindow` so theo GIÁ TRỊ thời gian, không so chuỗi.** Mốc cắt do JS dựng kết thúc bằng `Z`, còn `created_at` của PostgREST kết thúc bằng `+00:00`; so chuỗi thì `'…Z' > '…+00:00'` luôn đúng theo bảng mã ⇒ cả ngày doanh thu rơi nhầm ca. Đây là bẫy duy nhất của đợt này, đừng đổi lại.
- **Hệ quả có ý thức:** ngày chỉ mở Ca 1 (không ai mở Ca 2) thì phần bán sau 15:15 **không nằm trong ảnh ca nào** — vẫn đủ trong ảnh Tổng ngày (Tổng ngày đọc theo `business_date`). Bản cũ cho ca duy nhất ôm trọn ngày; test ghi rõ hành vi mới.
- Bù lại: ca trưởng chốt sớm/chốt muộn **không còn dịch chuyển doanh thu** giữa hai ca — đây là thứ BUG-117 (§46) chưa giải quyết được vì điểm cắt vẫn là giờ bấm nút.

### 50.2. Báo cáo gửi theo hẹn giờ, bàn giao không còn kích hoạt (`src/lib/reportSchedule.ts` mới)
- `REPORT_DUE_TIMES = { 1: '15:15', 2: '22:15' }`. `reconcileScheduledReport(user)` (idempotent) đọc `bag_shift_sessions` + `report_snapshots` của ngày, lấy ca **tới hạn mà chưa có `payload.shiftReports[shiftId]`**, sequence tăng dần (Ca 1 trước, vì Ca 2 luôn kèm Tổng ngày và Tổng ngày chốt sau cùng), rồi ghi `handoverReportRequest` với `trigger: 'schedule'`.
- **Ảnh báo cáo chỉ dựng được trong trình duyệt** (poster ẩn + html2canvas ở `ReportPage`), nên `App.tsx` chạy vòng 60s + một hẹn giờ đặt đúng 15:15:00 / 22:15:00 theo **đồng hồ máy chủ** (`src/lib/serverClock.ts` mới — `/api/server-time`, lưu độ lệch, bù nửa RTT, mất mạng thì rơi về đồng hồ máy). Tới giờ → `setPage('report')` → hiệu ứng tự chốt sẵn có làm nốt. Không ai mở app đúng giờ thì cờ vẫn còn, lần mở app kế tiếp gửi bù.
- Điều hướng ép sang màn Báo cáo **chỉ khi cờ vừa được đặt**, hoặc ở lượt chạy ĐẦU TIÊN của phiên app (`state.firstRun`). Không thì người đang bán bị kéo khỏi POS mỗi phút khi một lần gửi còn dở.
- `ReportPage` nới đúng 3 chỗ cho `trigger === 'schedule'` (`scheduledFinalization` / `freshScheduledFinalization`): (1) `resolveLeaderShiftSession` **không xét quyền sở hữu ca** — 15:15 là đúng lúc ca trưởng Ca sáng tan ca và tắt máy, bắt phải là chủ ca thì báo cáo Ca 1 không bao giờ gửi được; (2) không đòi ca `status === 'closed'`; (3) không đòi `openShiftCount === 0` khi chốt Ca 2. Đường **bấm tay giữ nguyên ràng buộc cũ**.
- `ShiftHandoverPage` **không còn ghi `handoverReportRequest` khi chốt ca** và không còn điều hướng sang Báo cáo. Lý do bắt buộc: chốt Ca 1 lúc 13:00 mà gửi luôn thì ảnh Ca 1 thiếu doanh thu 13:00→15:15 (vùng của ca giờ tính tới 15:15). Nút "Chốt báo cáo" thủ công ở `ReportPage` vẫn còn làm lối thoát.
- `AppShell` cho **cả ca phó** thấy thanh nhắc báo cáo (khớp `canRunScheduledReport`), cùng lý do (1) ở trên.
- `api/n8n-report-image.ts`: `REPORT_SEND_TIMES['shift-2']` đổi `22:00 → 22:15` cho khớp `REPORT_DUE_TIMES`. Đường hẹn giờ luôn `sendNow=true` nên không đi qua bảng này; sửa để hai lịch không nói hai kiểu.
- **Lưu ý vận hành:** chốt Ca 2 theo hẹn giờ gọi `finalizeDailyReport` ⇒ **đóng luôn ngày vận hành lúc 22:15**, và RPC tự đóng phiên ca còn mở (chỉ đổi `status`, KHÔNG ghi `closing_balances`, KHÔNG sinh phiếu `count` — không bịa số kiểm đếm). Bán muộn sau 22:15 phải bấm "Mở lại ngày".

**QA:** `tsc -b` + `npm run build` pass (`PRODUCTION_SUPABASE_BUNDLE_OK`). Test mới `scripts/test-report-schedule.mjs` (`REPORT_SCHEDULE_OK`); `scripts/test-shift-report-scope.mjs` viết lại cho luật đồng hồ (`SHIFT_REPORT_SCOPE_OK`, có ca chốt sớm/chốt muộn + timestamp kiểu PostgREST). Pass lại: COMPETITION_DRILLDOWN_OK, N8N_REPORT_IMAGE_QUEUE_OK (sửa 2 kỳ vọng stale: lịch `shift-2` và chống-trùng bám `force` chứ không bám `sendNow`), SHIFT_REALTIME_REMINDERS, HANDOVER_SHIFT_RECOVERY_OK, SHIFT_OWNER_MUST_BE_LEADER, SHIFT_SEQUENCE_BY_WINDOW, AUTO_SECOND_SHIFT_START, ALLDAY_REGISTRATION_NOT_STEAL_SHIFT.
- **ĐỎ SẴN từ trước, không liên quan:** `test-operations-ux-performance.mjs` đòi gỡ nút "Gửi Zalo"/"Chia sẻ ảnh Zalo" thủ công ở `ReportPage` — hai nút đó vẫn còn từ HEAD và vẫn là lối thoát khi n8n hỏng.

## 51. Vào sớm 15 phút + doanh thu ca trưởng theo ca làm (2026-08-11) — CHƯA DEPLOY
> Chủ quán chốt: "ca 9-13h thì bắt buộc phải vô trước 15 phút, không vô sớm thì tính đi trễ" + "doanh thu ca trưởng ghi nhận theo dạng ca làm... nhưng ca trưởng bấm bill thì vẫn ghi nhận là bán hàng" + "KPI ca trưởng khoanh đi, chỉ ghi nhận số".

**Cả hai luật chỉ áp cho dữ liệu TỪ 01/08/2026.** Tháng 7 đã chốt lương, đã gửi báo cáo Zalo — đổi ngược về quá khứ là hàng loạt người bỗng thành đi trễ và số thi đua đã gửi bị lệch.

### 51.1. Đi trễ tính từ mốc VÀO SỚM 15 PHÚT (`src/lib/attendanceLateness.ts` mới)
- `onTimeCheckInDeadline(workDate, scheduledStart, grace)`: ca từ `EARLY_CHECK_IN_REQUIRED_FROM = '2026-08-01'` ⇒ **giờ vào ca − 15 phút, BỎ dung sai**; ca cũ hơn giữ luật cũ (giờ vào ca + dung sai của ca). Ca 09:00–13:00 ⇒ hạn 08:45; bấm 08:46 trễ 1 phút, bấm đúng 09:00 trễ **15** phút.
- **Bỏ dung sai là cố ý:** dung sai 5 phút chồng lên mốc mới sẽ biến "trước 15 phút" thành "trước 10 phút" — nói một đằng tính một nẻo. `graceMinutesByShift` vẫn còn cho ca trước mốc.
- Áp tại `attendance.ts`: `buildAttendanceReport` (`lateCount` ⇒ `isLateCheckIn`) và `buildAttendanceDetailRows` (`lateMinutes` ⇒ `lateMinutesFor`). Hai hàm này nuôi bảng công, Excel chấm công, và trang Xem công.
- **UI phải nói trước, không để nhân viên bấm xong mới biết:** thẻ ca ở `AttendancePage` hiện dòng "Check-in trước 08:45 (sớm 15 phút) để không bị tính đi trễ", đổi sang nền đỏ + "check-in bây giờ sẽ bị tính đi trễ" khi đã quá mốc (`.attendance-early-hint` / `.attendance-early-hint.late`). `MyTimesheetPage` đổi câu "trễ so với giờ vào ca" thành "ca này phải check-in trước giờ vào 15 phút".
- **Hệ quả cần theo dõi:** thưởng chuyên cần tháng đòi `lateCount === 0` (`commission.ts`), luật siết hơn ⇒ ít người đạt hơn.

### 51.2. Doanh thu ca trưởng = tổng các CA mình làm (`buildShiftLeaderRecordedRevenue`)
- Lý do: ca trưởng đứng quầy điều phối chứ hiếm khi bấm bill — **tháng 7/2026 hai ca trưởng Vũng Tàu có ĐÚNG 0 hóa đơn đứng tên** (kiểm trên DB prod). Luật "KPI đội" chỉ vá được nửa đầu tháng 7, nửa sau doanh thu về 0 và hạng khóa cứng ở D.
- Công thức: doanh thu các ca mình đứng tên `bag_shift_sessions.leader_id` **∪** hóa đơn chính mình bấm ở ca người khác. Gom bằng **tập id hóa đơn** — bill ca trưởng bấm trong ca của chính mình đã nằm trong tổng ca, không được cộng hai lần.
- Ranh giới ca lấy từ `sessionScopeWindow` (mốc đồng hồ 15:15 của §50) ⇒ tổng các ca trong ngày luôn bằng tổng ngày. Loại `isKpiAdjustmentReceipt` khỏi tổng ca: phiếu "Bổ sung KPI lịch sử" không thuộc ca nào.
- **Ca trưởng CHƯA bị chấm KPI** (`recordedLeader` trong `buildCommissionRows`): `targetQuantity = 0`, `progress = 0`, `rank = ''`, `dailyBonus = 0`, `commission = 0`. Giữ chỉ tiêu cũ thì vô nghĩa — doanh thu theo ca lớn gấp ~3 lần chỉ tiêu cá nhân (VT tháng 7: Ngân 32,4tr / Tú 32,0tr so với chỉ tiêu 10,008tr) nên ai cũng vượt 200%. **Khi chủ quyết chỉ tiêu thì bỏ cờ này**, mốc nằm ở `LEADER_SHIFT_REVENUE_FROM` / `usesLeaderShiftRevenue()` trong `commission.ts`.
- Bảng thi đua hiện `—` + "Chưa chấm KPI" thay cho hạng D 0% (`competition-classification` và poster) — hạng D khi chưa chấm đọc như bị đánh giá kém.
- **Ca trưởng không có hóa đơn nào vẫn phải có dòng:** thêm bước seed row trong `buildCommissionRows` theo map doanh thu-theo-ca, nếu không họ biến mất khỏi bảng dù đứng tên hàng chục ca.
- Luật KPI đội Vũng Tàu (`usesVungTauNewKpi`, 01–15/07) **giữ nguyên** cho lịch sử; từ 01/08 nó không còn kích hoạt nên không đụng nhau.

**QA:** `tsc -b` + `npm run build` pass (`PRODUCTION_SUPABASE_BUNDLE_OK`). Test mới `scripts/test-early-checkin-and-leader-shift-revenue.mjs` (`EARLY_CHECKIN_AND_LEADER_SHIFT_REVENUE_OK` — test bằng số cả mốc trễ lẫn chống cộng trùng hóa đơn). Pass lại: ATTENDANCE_LATE_CHECKOUT, ATTENDANCE_SINGLE_OPEN_SESSION, DEPUTY_AND_ATTENDANCE_RULES, BUSINESS_COMPETITION_KPI, EMPLOYEE_COMPETITION_BOARD, EMPLOYEE_SALES_CAPACITY, COMPETITION_DRILLDOWN, KPI_REVENUE_ADJUSTMENTS, BRANCH_KPI_SETTINGS, SHIFT_REPORT_SCOPE, REPORT_SCHEDULE.
- **4 script sửa kỳ vọng stale** (do đổi `const kpiBonus = row.dailyBonus` → `dailyBonus` có ép 0 cho ca trưởng): `test-kpi-ranking-reward-clarity`, `test-vung-tau-kpi-20260810`, `test-daily-only-kpi-reward-20260810`, `test-kpi-reward-reverse-audit`. Ý nghĩa kiểm tra giữ nguyên (tiền KPI chỉ đến từ thưởng ngày).

### 51.3. KPI ca trưởng do CHỦ tự đặt trên web (bổ sung cùng ngày)
- `hasLeaderKpiTarget(branchId)` (`commission.ts`) đọc override `branch_kpi_formulas` cho vị trí `shift_leader`. **Chưa đặt (0) ⇒ chỉ ghi nhận doanh thu**, không %, không hạng, không thưởng. **Đặt rồi ⇒ chấm KPI ngay, không cần build lại app.** Chỗ đặt: Quản trị → Thi đua nhân viên → "Mức KPI theo chi nhánh".
- KHÔNG dùng `POSITION_KPI_FORMULAS` làm mốc cho ca trưởng nữa: mức đó dành cho thời "doanh thu = hóa đơn tự bấm", nhỏ hơn doanh thu theo ca ~3 lần ⇒ ai cũng vượt 200%.
- Có mức rồi thì **chấm cả KPI ngày**: `buildShiftLeaderRecordedRevenue` trả thêm `days[]`, AdminPage gọi `applyDailyResult` theo từng ngày. Không có bước này thì ca trưởng có hạng nhưng thưởng luôn bằng 0 (họ không sinh `dailyPerformance` vì không đứng tên hóa đơn nào).
- **Excel đối chiếu đã khớp:** `buildShiftLeaderRecordedRevenue` trả thêm `sources[]` = ĐÚNG bộ hóa đơn đã cộng vào doanh thu; `buildCompetitionSourcesByRow` đọc thẳng từ đó cho ca trưởng thay vì dò lại theo `seller_id`. Trước bản này ca trưởng luôn ra "Số nguồn 0 · Đối chiếu 0đ · Lệch" (đúng lỗi thấy trong file Excel 11/08/2026). Nguồn ghi rõ "Ca N do mình phụ trách" hay "Tự bấm ngoài ca".
- Một hàm duy nhất sinh cả doanh thu lẫn nguồn đối chiếu ⇒ hai con số không thể lệch nhau.
