-- Keep a schedule edit and its existing attendance record on one registration.
-- Changing a main shift after check-in updates the attended registration in place
-- instead of preserving it and inserting a second main registration for the day.
...
declare
  ...
  v_attended_registration public.shift_registrations;
...
begin
  ...
  if v_attended_count > 1 then
    raise exception 'Co nhieu ca chinh da cham cong trong ngay; can Admin doi chieu';
  end if;

  if v_attended_count = 1 then
    select * into v_attended_registration
    from public.shift_registrations sr
    where sr.user_id = p_user_id
      and sr.branch_id = p_branch_id
      and sr.work_date = p_work_date
  ...
  where sr.user_id = p_user_id
    and sr.branch_id = p_branch_id
    and sr.work_date = p_work_date
    and sr.id is distinct from v_attended_registration.id
    and not exists (
      select 1 from public.attendance_records ar
      where ar.shift_registration_id = sr.id
    )
  ...
  if p_shift_id is null and (v_start is null or v_end is null) then
    if v_attended_count = 1 then
      raise exception 'Ca da check-in; hay chon ca dung thay vi chuyen OFF';
    end if;
    return null;
  end if;
  ...
  if v_attended_count = 1 then
    ...
    where id = v_attended_registration.id
