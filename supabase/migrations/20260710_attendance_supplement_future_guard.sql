-- Prevent admin supplements from pre-filling attendance for a shift that has
-- not ended yet. Such records hide the real check-in action from employees.

-- Repair only impossible pre-filled records: a completed attendance record
-- whose declared check-out was still in the future when the row was created,
-- with created_at and updated_at written in the same insert. Normal check-outs
-- update the row later and are therefore preserved.
...
begin
  ...
  if v_check_out > now() then
    raise exception 'Chỉ được bổ sung công sau khi ca đã kết thúc';
  end if;
